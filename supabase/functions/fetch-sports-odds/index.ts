...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      // Step 1: /schedule?year=
      const scheduleRes = await fetch(
        `${GOLF_BASE}/schedule?year=${currentYear}`,
        { headers: golfHeaders, signal: AbortSignal.timeout(8000) },
      );
      console.log("[golf-discovery] schedule status:", scheduleRes.status);
      let tournaments: any[] = [];
      if (scheduleRes.ok) {
        const scheduleJson = await scheduleRes.json();
        const top = Array.isArray(scheduleJson) ? scheduleJson[0] : scheduleJson;
        console.log(
          "[golf-discovery] schedule keys:",
          top && typeof top === "object" ? Object.keys(top) : typeof top,
        );
        console.log(
          "[golf-discovery] schedule preview:",
          JSON.stringify(scheduleJson).slice(0, 800),
        );
        tournaments = Array.isArray(scheduleJson)
          ? scheduleJson
          : scheduleJson.schedule ?? scheduleJson.tournaments ?? [];
        console.log("[golf-discovery] total tournaments:", tournaments.length);
        console.log(
          "[golf-discovery] first 3 tourneys:",
          JSON.stringify(tournaments.slice(0, 3)),
        );
        const theOpen = tournaments.find((t: any) => {
          const s = JSON.stringify(t).toLowerCase();
          return s.includes("open") || s.includes("british");
        });
        console.log("[golf-discovery] The Open entry:", JSON.stringify(theOpen));

        // Step 2: /leaderboard
        const firstTourney = tournaments[0];
        const tournId =
          firstTourney?.tournId ?? firstTourney?.id ?? firstTourney?.tournamentId;
        console.log("[golf-discovery] first tournId:", tournId);
        if (tournId) {
          const lbRes = await fetch(
            `${GOLF_BASE}/leaderboard?year=${currentYear}&tournId=${tournId}`,
            { headers: golfHeaders, signal: AbortSignal.timeout(8000) },
          );
          console.log("[golf-discovery] leaderboard status:", lbRes.status);
          if (lbRes.ok) {
            const lbJson = await lbRes.json();
            const lbTop = Array.isArray(lbJson) ? lbJson[0] : lbJson;
            console.log(
              "[golf-discovery] leaderboard keys:",
              lbTop && typeof lbTop === "object" ? Object.keys(lbTop) : typeof lbTop,
            );
            console.log(
              "[golf-discovery] leaderboard preview:",
              JSON.stringify(lbJson).slice(0, 800),
            );
          } else {
            console.log(
              "[golf-discovery] leaderboard error:",
              (await lbRes.text()).slice(0, 200),
            );
          }
        }
      } else {
        console.log(
          "[golf-discovery] schedule error:",
          (await scheduleRes.text()).slice(0, 200),
        );
      }

      // Step 3: /stats — try common statIds
      const statIds = ["02674", "02567", "02564", "101", "102", "103"];
      for (const statId of statIds) {
        const statRes = await fetch(
          `${GOLF_BASE}/stats?year=${currentYear}&statId=${statId}`,
          { headers: golfHeaders, signal: AbortSignal.timeout(8000) },
        );
        console.log(`[golf-discovery] stats ${statId}:`, statRes.status);
        if (statRes.ok) {
          const statJson = await statRes.json();
          console.log(
            `[golf-discovery] stats ${statId} preview:`,
            JSON.stringify(statJson).slice(0, 400),
          );
          break;
        } else {
          console.log(
            `[golf-discovery] stats ${statId} error:`,
            (await statRes.text()).slice(0, 120),
          );
        }
        await new Promise((r) => setTimeout(r, 200));
      }

      // Step 4: /players?lastName=
      const testPlayers = ["Scheffler", "McIlroy", "Schauffele"];
      for (const lastName of testPlayers) {
        const playerRes = await fetch(
          `${GOLF_BASE}/players?lastName=${lastName}`,
          { headers: golfHeaders, signal: AbortSignal.timeout(8000) },
        );
        console.log(`[golf-discovery] player ${lastName}:`, playerRes.status);
        if (playerRes.ok) {
          const playerJson = await playerRes.json();
          console.log(
            `[golf-discovery] player ${lastName} preview:`,
            JSON.stringify(playerJson).slice(0, 400),
          );
          break;
        } else {
          console.log(
            `[golf-discovery] player ${lastName} error:`,
            (await playerRes.text()).slice(0, 120),
          );
        }
        await new Promise((r) => setTimeout(r, 200));
      }
